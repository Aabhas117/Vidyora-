...
import { getMyVideos, updateVideoOnServer, deleteVideoOnServer} from "../Services/videoService";
...
export default function Profile() {
  ...
  const handleDeleteVideo = async (videoId) => {
    const previous = myVideos;
    setMyVideos((prev) => prev.filter((v) => v.id !== videoId));
    try {
      await deleteVideoOnServer(videoId);
    } catch {
      setMyVideos(previous); // roll back if the real delete failed
    }
  };

  const handleSaveEditedVideo = async (videoId, patch) => {
    try {
      const updated = await updateVideoOnServer(videoId, patch);
      setMyVideos((prev) => prev.map((v) => (v.id === videoId ? updated : v)));
    ...
    <div className="max-w-2xl">
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6">
        {editing ? (
          <form onSubmit={handleSave} className="flex flex-col gap-4">
            <div className="flex items-center gap-4">
              <div className="relative">
                <Avatar src={avatarPreview} name={form.fullName} className="h-20 w-20" />
          ...
          <>
            <div className="flex items-center gap-4">
              <Avatar src={user.avatar} name={user.fullName} className="h-20 w-20" />
              <div>
            ...
            <div className="flex gap-8 mt-6 pt-6 border-t border-zinc-800">
              <div>
                <p className="text-lg font-semibold text-zinc-100">{myVideos.length}</p>
